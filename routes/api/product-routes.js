...
router.get('/', async (req, res) => {
  ...
  router.get('/:id', async (req, res) => {
    ...
    try {
      ...
      if(!products) {
        res.status(400).json({ message: 'Sorry something is wrong with this id, please try again'});
        return;    
      }
...
router.delete('/:id', async (req, res) => {
  ...
  try {
    const ProductTag = await Product.destroy({ where: {id:req.params.id}})
